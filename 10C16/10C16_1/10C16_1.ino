...
#define SERVO_MOVE_TIME 1000.0
Servo servo;
unsigned long servoTime=0;
bool servoTimeCheck=false,servoOpened=false;
float servoAngle=SERVO_CLOSE,servoStart=SERVO_CLOSE,servoTarget=SERVO_CLOSE;
...
void updateServo(){
  float t=(millis()-servoMoveStart)/SERVO_MOVE_TIME;
  if(t>1.0) t=1.0;
  float x=(t-0.5)*12.0;
  t=1.0/(1.0+exp(-x));
  servoAngle=servoStart+(servoTarget-servoStart)*t;
